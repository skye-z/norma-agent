...
const MessageTimingBadge: React.FC = () => {
  ...
  if (!timing || Number.isNaN(timing.totalStreamTime) || !timing.totalStreamTime) return null;
  ...
  return (
    <span className="text-[9px] text-norma-textDim font-mono">
      {main}
      {timing.firstTokenTime && !Number.isNaN(timing.firstTokenTime) ? ` · TTFT ${formatMs(timing.firstTokenTime)}` : ""}
      {timing.tokensPerSecond && !Number.isNaN(timing.tokensPerSecond) ? ` · ${Math.round(timing.tokensPerSecond)} tok/s` : ""}
    </span>
  );
};
...
const LoadingIndicator: React.FC = () => {
  ...
  const hasText = (message as any).content?.some((p: any) => p.type === "text" && p.text?.length > 0);
  const isRunning = thread.isRunning;
  if (!isRunning || hasText) return null;
  return (
    <div className="flex items-center gap-1.5 py-1">
      <div className="w-1 h-1 rounded-full bg-norma-accent animate-bounce" style={{ animationDelay: "0ms" }} />
      <div className="w-1 h-1 rounded-full bg-norma-accent animate-bounce" style={{ animationDelay: "150ms" }} />
      <div className="w-1 h-1 rounded-full bg-norma-accent animate-bounce" style={{ animationDelay: "300ms" }} />
        ...
        <div className="flex items-center gap-1 absolute -bottom-5 left-0 right-0 px-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none group-hover:pointer-events-auto">
          ...
          <ActionBarPrimitive.Root hideWhenRunning autohide="not-last" className="flex gap-0.5">
            ...
            <ActionBarPrimitive.Reload className="win-btn !w-4 !h-4" title="重新生成">
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16"/><path d="M3 21v-5h5"/></svg>
