...
const ICONS: Record<string, React.ReactNode> = {
  screen: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="3" width="20" height="14" rx="2" ry="2" /><line x1="8" y1="21" x2="16" y2="21" /><line x1="12" y1="17" x2="12" y2="21" /></svg>,
  action: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2" /></svg>,
  window: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="3" width="20" height="14" rx="2" /><line x1="8" y1="21" x2="16" y2="21" /></svg>,
  ...
  tray: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="6 9 12 15 18 9" /></svg>,
  ...
  default: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="16 18 22 12 16 6" /><polyline points="8 6 2 12 8 18" /></svg>,
};
...
const CapabilitiesPage: React.FC = () => {
  ...
  useEffect(() => {
    window.electronAPI?.getCapabilities?.().then((tools: Array<{ id: string; name: string; description: string; category: string }>) => {
      if (!tools || tools.length === 0) return;
      const metaRecord: Record<string, any> = {};
      const caps: Capability[] = tools.map((t) => {
        const iconKey = ICON_MAP[t.id] || "default";
        metaRecord[t.id] = { name: t.name, description: t.description, category: t.category };
        return {
          id: t.id,
          name: t.name,
          desc: t.description,
    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      <div className="flex-1 overflow-y-auto px-5 py-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          {filtered.map((cap) => (
            <div key={cap.id} className="rounded-xl bg-white/[0.03] border border-white/[0.06] px-4 py-3 hover:border-white/[0.1] transition-colors group">
              ...
              <div className="text-[10px] text-norma-textMuted mb-1">{cap.desc}</div>
              <div className="flex items-center gap-2">
                <span className="text-[9px] text-norma-textDim font-mono">{cap.id}</span>
                <span className="text-[9px] px-1 py-0.5 rounded bg-white/[0.04] text-norma-textDim">{cap.category}</span>
              </div>
            </div>
          ))}
        </div>
