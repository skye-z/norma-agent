...
const MemoryTab: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setWorkingMemory(wm);
      let msgCount = 0;
      const enriched = await Promise.all(threadList.map(async (t: ThreadInfo) => {
        try {
          const msgs = await window.electronAPI?.getThreadMessages?.(t.id);
          msgCount += msgs?.length ?? 0;
          if ((!t.title || t.title === "新会话") && Array.isArray(msgs) && msgs.length > 0) {
            const firstUser = msgs.find((m: any) => m.role === 'user');
            const preview = firstUser?.content?.text || firstUser?.content || '';
            if (typeof preview === 'string' && preview.trim()) {
              return { ...t, title: preview.trim().slice(0, 30) + (preview.trim().length > 30 ? '...' : '') };
            }
          }
          return t;
        } catch { return t; }
  ...
  const isMemoryEmpty = !workingMemory || workingMemory.trim().length === 0 || (workingMemory.trim().startsWith('# 用户档案') || workingMemory.trim().startsWith('# User Profile')) && !workingMemory.includes('- **');
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[13px] font-semibold text-norma-text">工作记忆</span>
            </div>
            {isMemoryEmpty ? (
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[13px] font-medium text-norma-text">会话记录</span>
            </div>
...
const _embedderBaseUrl = (s: { configured: boolean; baseUrl?: string }) =>
  s.baseUrl ? s.baseUrl.replace(/^https?:\/\//, '').replace(/\/v\d+$/, '') : 'unknown';
...
const KnowledgeTab: React.FC = () => {
  ...
  const handleIngestFile = async () => {
    try {
      ...
      const tasks: ImportTask[] = res.files.map((f, i) => ({
  ...
  return (
    <div className="flex-1 overflow-y-auto scrollbar-show px-5 py-4 relative">
      ...
      {embedderStatus?.configured && (
        <div className="mb-3 text-[11px] text-norma-textDim">
          Embedding: {embedderStatus.mode === 'local' ? '本地 (384维)' : _embedderBaseUrl(embedderStatus)}
        </div>
      )}
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => { setShowUpload(false); setUploadName(""); setUploadText(""); }}>
          <div className="w-[480px] max-h-[70vh] rounded-2xl bg-[#1c1c20] border border-white/[0.08] shadow-2xl flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-2 px-4 py-3 border-b border-white/[0.06]">
              <span className="text-[15px] font-medium text-norma-text flex-1">添加文档到知识库</span>
              <button
            ...
            <div className="flex-1 overflow-y-auto scrollbar-show p-4 space-y-3">
              <div>
                <div className="text-[12px] text-norma-textDim mb-1">文档名称</div>
                <input
                  value={uploadName}
              ...
              <div>
                <div className="text-[12px] text-norma-textDim mb-1">文档内容</div>
                <textarea
                  value={uploadText}
          ...
          <div className="flex flex-col gap-2">
            {searchResults.map((r, i) => (
              <div key={r.id} className="rounded-xl bg-white/[0.03] border border-norma-accent/20 px-4 py-3">
                <div className={`text-[13px] text-norma-text leading-relaxed whitespace-pre-wrap ${expandedResult === i ? '' : 'line-clamp-4'}`}>
            ...
            {searchResults.length === 0 && (
              <div className="text-[13px] text-norma-textDim text-center py-4">未找到相关内容</div>
            )}
          </div>
        </div>
      )}
        ...
        {importTasks.map((task) => (
          ...
          >
            {task.status === 'processing' ? (
              ...
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-green-400 flex-none">
                <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" />
                <path d="m9 11 3 3L22 4" />
              </svg>
            ) : task.status === 'error' ? (
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-red-400 flex-none">
                <circle cx="12" cy="12" r="10" />
                <line x1="15" y1="9" x2="9" y2="15" />
                <line x1="9" y1="9" x2="15" y2="15" />
              </svg>
            ) : (
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-norma-textDim flex-none">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[11px] text-norma-textDim mt-0.5">
                ...
                {task.status === 'done' && `${task.chunks || 0} 分块 · 导入完成`}
            ...
            {(task.status === 'done' || task.status === 'error') && (
              <button
                onClick={() => setImportTasks((prev) => prev.filter((t) => t.id !== task.id))}
