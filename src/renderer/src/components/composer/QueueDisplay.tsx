...
export const QueueDisplay: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-1.5 mb-1 w-full">
      {queue.map((msg, idx) => (
        <div key={idx} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/[0.03] border border-white/[0.06] text-[11px] group">
          ...
          {editingIdx === idx ? (
            ...
            <>
              ...
              <button
                ...
                className="opacity-0 group-hover:opacity-100 flex-none px-2 py-0.5 rounded bg-norma-accent/20 text-norma-accent hover:bg-norma-accent hover:text-white transition-all duration-200 text-[10px]"
                title="打断并发送"
              >
                立即发送
              </button>
