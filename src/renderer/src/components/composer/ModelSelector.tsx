...
const ModelSelector: React.FC = () => {
  ...
  const [models, setModels] = useState<Array<{ modelId: string; displayName: string; providerName: string; providerPreset: string; providerId: string }>>([]);
  const [selectedIdx, setSelectedIdx] = useState(0);

  const buildModelString = (model: { modelId: string; providerPreset: string }) => {
    const providerType = model.providerPreset === 'custom' ? 'openai' : model.providerPreset;
    return `${providerType}/${model.modelId}`;
  };

  useEffect(() => {
    (window as any).electronAPI?.configGet?.("norma-providers").then((providersRaw: any) => {
      const providers: SavedProviderFull[] = Array.isArray(providersRaw) ? providersRaw : [];
      return (window as any).electronAPI?.configGet?.("norma-enabled-models").then((enabledRaw: any) => {
        const enabled: EnabledModel[] = Array.isArray(enabledRaw) ? enabledRaw : [];
        if (providers.length === 0 || enabled.length === 0) return;

        (window as any).electronAPI?.configGet?.("norma-cached-models").then((cachedRaw: any) => {
          const cached: Record<string, any[]> = cachedRaw && typeof cachedRaw === 'object' ? cachedRaw : {};

          const result = enabled.map((em) => {
            const prov = providers.find((p) => p.id === em.providerId);
            const modelCache = cached[em.providerId] || [];
            const modelInfo = modelCache.find((m: any) => m.id === em.modelId);
            return {
              modelId: em.modelId,
              displayName: modelInfo?.display_name || modelInfo?.name || em.modelId,
  ...
  useEffect(() => {
    ...
    (window as any).electronAPI?.getActiveModel?.().then((activeModel: string | null) => {
      if (activeModel) {
        const idx = models.findIndex((m) => buildModelString(m) === activeModel);
  ...
  return (
    <div className="relative flex-none">
      ...
      >
        <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: PROVIDER_COLORS[selected.providerPreset] || "#8b8b8b" }} />
        ...
        <>
          ...
          <div className="absolute bottom-full left-0 mb-2 w-56 rounded-xl bg-[#1a1a1f] border border-white/[0.08] shadow-2xl z-50 overflow-hidden">
            <div className="px-3 py-2 border-b border-white/[0.06]">
              <span className="text-[9px] text-norma-textDim uppercase tracking-wider">已启用模型</span>
            </div>
            {models.map((model, idx) => (
              ...
              >
                <span className="w-2 h-2 rounded-full flex-none" style={{ backgroundColor: PROVIDER_COLORS[model.providerPreset] || "#8b8b8b" }} />
                <div className="flex-1 text-left min-w-0">
                  <div className={`font-medium truncate ${idx === selectedIdx ? "text-norma-accent" : "text-norma-text"}`}>
                    {model.displayName}
                  </div>
                  <div className="text-[9px] text-norma-textDim font-mono">{model.modelId}</div>
                </div>
                {idx === selectedIdx && (
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="text-norma-accent flex-none">
